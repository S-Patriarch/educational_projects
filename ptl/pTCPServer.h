/*++

    Copyright (c) S-Patriarch, 2023
    (PTL) Patriarch library : pTCPServer.h

Методы:
    setup() - создание сокета, установка сервера
    Send() - передача данных
    receive() - чтение данных строкой определенного размера
    read() - чтение данных по символьно
    exit() - закрытие сокета, завершение соединения

--*/

#ifndef __PTL_TCP_SERVER_H__
#define __PTL_TCP_SERVER_H__

#include <iostream>
#include <cstring>
#include <unistd.h>
#include <sys/types.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <cstdint>

#include "pexcept.h"

//////////////////////////////////////////////////////////////////////
namespace ptl
{
  class pTCPServer
  {
    private:
      std::uint16_t _M_port          = 0;
      std::uint16_t _M_connectClient = 7;

      std::int32_t  _M_sock          = -1; 
      std::int32_t  _M_connection    = -1;
      std::int32_t  _M_maxPacketSize = 4096;

      struct sockaddr_in server;
      struct sockaddr_in client;

    public:

//--------------------------------------------------------------------
// Создание сокета, установка сервера.
// При успешной отработке возвращает true.
// В противном случае - бросает исключения.
//
      bool setup(std::uint16_t _port)
      {
        if (_M_sock == -1) {
          _M_sock = socket(AF_INET, SOCK_STREAM, 0);
          if (_M_sock == -1) {
            throw
            ptl::pException("E: Не удалось создать сокет.");
          }
        }

        _M_port = _port;

        std::memset(&server, 0, sizeof(server));
        
        server.sin_addr.s_addr = htonl(INADDR_ANY);
        server.sin_port        = htons(_M_port);
        server.sin_family      = AF_INET;

        std::int32_t _bindStatus = bind(_M_sock,
                                        (struct sockaddr*)&server,
                                        sizeof(server)
                                   );
        if (_bindStatus == -1) {
          throw
          ptl::pException("E: Не удалось выполнить привязку сокета."); 
        }

        std::int32_t _connectionStatus = listen(_M_sock, _M_connectClient);
        if (_connectionStatus == -1) {
          throw
          ptl::pException("E: Сокет не может прослушивать новые подключения."); 
        }

        std::memset(&client, 0, sizeof(client));

        socklen_t _sosize = sizeof(client);
        _M_connection = accept(_M_sock,
                               (struct sockaddr*)&client,
                               &_sosize
                        );
        if (_M_connection == -1) {
          throw
          ptl::pException("E: Сервер не может принять данные от клиента."); 
        }

        return true;
      }

//--------------------------------------------------------------------
// Передача данных через созданный сокет.
//
      void Send(std::string _data)
      { send(_M_connection, _data.c_str(), std::strlen(_data.c_str()), 0); }

//--------------------------------------------------------------------
// Чтение (получение) данных по символьно.
// При успешном чтении (получении) возвращает std::string
// В противном случае - nullptr
//
      std::string read()
      {
        std::string _reply {};
        char        _buffer[_M_maxPacketSize];

        std::memset(&_buffer[0], 0, sizeof(_buffer));

        if (recv(_M_connection, _buffer, _M_maxPacketSize, 0) < 0) {
          return nullptr;
        }

        _reply = _buffer;

        return _reply;
      }

//--------------------------------------------------------------------
// Чтение (получение) данных строкой определенного размера.
// При успешном чтении (получении) возвращает std::string
// В противном случае - nullptr
//
      std::string receive()
      {
        std::string _reply {};
        char        _buffer[_M_maxPacketSize];

        std::memset(&_buffer[0], 0, sizeof(_buffer));

        if (recv(_M_connection, _buffer, _M_maxPacketSize, 0) < 0) {
          return nullptr;
        }

        _buffer[_M_maxPacketSize-1] = '\0';
        _reply                      = _buffer;

        return _reply;
      }

//--------------------------------------------------------------------
// Закрытие созданного сокета и 
// завершение установленного с сервером соединения.
//
      void exit()
      {
        close(_M_connection); 
        close(_M_sock); 
      }

  };

} // namespace ptl

#endif // __PTL_TCP_SERVER_H__